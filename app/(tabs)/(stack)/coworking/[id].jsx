...
export default function CoWorkingDetailScreen() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      console.log("Aqui recibimos: ", data);
      setSpace(data);
      setLoading(false);
    };
    load();
      ...
      >
        <View className="mx-4 bg-card rounded-nexus shadow-nexus p-5">
          ...
          <View className="mb-4">
            ...
            <Text
              className={`font-inter ${space.ocupado ? "text-red-500" : "text-emerald-500"}`}
            >
          ...
          {space.servicios && (
            <View className="mb-4">
              ...
              <Text className="font-inter text-secondary">
                {Array.isArray(space.servicios)
                  ? space.servicios.join(", ")
                  : space.servicios}
              </Text>
            </View>
          )}
